  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        purchase_orders!inner ( po_number, order_date, status, tenant_id, companies ( name ) ),
...
router.get('/pending-by-vkn', async (req, res) => {
  try {
    ...
    const { data: company, error: companyError } = await supabase
      .from('companies').select('id').eq('vkn_tckn', vkn).eq('tenant_id', tenantId).single();
...
router.post('/', async (req, res) => {
  try {
    ...
    let { data: company } = await supabase.from('companies').select('id, name').eq('vkn_tckn', companyVkn).eq('tenant_id', tenantId).single();
    if (!company) {
      ...
      const { data: createdCompany, error: companyInsertErr } = await supabase
        .from('companies').insert({ vkn_tckn: companyVkn, name: companyName, tenant_id: tenantId }).select('id, name').single();
      if (companyInsertErr) throw companyInsertErr;
      company = createdCompany;
    }

    const uniqueCodes = [...new Set(items.map(x => x.product_code))];
    const { data: products, error: productsErr } = await supabase.from('products').select('id, product_code, product_name').in('product_code', uniqueCodes).eq('tenant_id', tenantId);
    ...
    if (missingCodes.length > 0 && forceCreate) {
      ...
      for (const code of missingCodes) {
        ...
        const { data: newProduct, error: insertErr } = await supabase
          .from('products').insert({ product_code: code, product_name: it?.product_name || code, brand: it?.brand || null, category: it?.category || null, tenant_id: tenantId })
    ...
    const { data: existingPoRows, error: poFetchErr } = await supabase.from('purchase_orders').select('po_number').eq('tenant_id', tenantId).ilike('po_number', `${poPrefix}-%`);
    ...
    let po = null, poErr = null, attemptSeq = maxSeq + 1;
    for (let attempt = 0; attempt < (inputPoNumber ? 1 : 5); attempt++) {
      const attemptPoNumber = inputPoNumber || `${poPrefix}-${attemptSeq}`;
      const result = await supabase.from('purchase_orders').insert({ po_number: attemptPoNumber, company_id: company.id, status: 'Bekliyor', tenant_id: tenantId }).select('id, po_number').single();
...
router.put('/items/:id', async (req, res) => {
  try {
    ...
    if (orderedQty < Number(existing.received_qty || 0)) return res.status(400).json({ error: `Sipariş miktarı ${existing.received_qty} altına düşemez.` });
...
router.delete('/items/:id', async (req, res) => {
  try {
    ...
    if ((linkedCount || 0) > 0) return res.status(400).json({ error: 'Bu kaleme bağlı fatura kaydı var.' });
